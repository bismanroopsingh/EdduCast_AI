...
# ------------------------------------------------------------
# FIX: cap how much text from a single PDF gets chunked/embedded
# at once. A very large document can spike memory hard during the
# embedding + FAISS-index step, which is exactly when the 512MB
# crash was happening on upload.
# ------------------------------------------------------------
MAX_CHARS = 50000  # roughly ~15-20 pages of typical text
...
# ------------------------------------------------------------
# FIX: process a PDF's bytes into {text, chunks, index, topics}.
# Used both for a fresh upload AND when re-activating a
# previously-uploaded file that only has its raw bytes cached
# (not its heavy processed data). Keeping this as one function
# means "switch back to an old file" and "upload a new file" both
# go through the exact same path.
# ------------------------------------------------------------
def process_pdf_bytes(file_bytes, file_name, user_id, document_id=None):

    text = extract_text_from_pdf(io.BytesIO(file_bytes))

    if len(text) > MAX_CHARS:
        st.warning(
            f"'{file_name}' is large — processing the first "
            f"{MAX_CHARS:,} characters only, to keep memory usage low on this demo."
        )
        text = text[:MAX_CHARS]

    if document_id is None:
        document_id = save_document(user_id, file_name, text)
...
if page == "Learn":
    ...
    # ------------------------------------------------------------
    # FIX: "library" now stores only LIGHTWEIGHT metadata for every
    # uploaded file (name, document_id, raw PDF bytes). The heavy
    # processed data (extracted text, chunks, FAISS index) is kept
    # ONLY for the currently active document, in "active_data".
    # This means uploading/switching between several files no
    # longer keeps every file's chunks+embeddings+index resident
    # in memory at once - only one document's worth at a time.
    # ------------------------------------------------------------
    if "library" not in st.session_state:
        st.session_state["library"] = {}   # fingerprint -> {name, document_id, raw_bytes}
    ...
    if uploaded_file is not None:
        ...
        if file_fingerprint not in st.session_state["library"]:
            ...
            with st.spinner(f"Processing {uploaded_file.name}..."):
                processed = process_pdf_bytes(
                    raw_bytes,
                    uploaded_file.name,
                    st.session_state["user_id"]
                )

            # Store only light metadata + raw bytes in the library
            st.session_state["library"][file_fingerprint] = {
                "name": uploaded_file.name,
            ...
            # The newly uploaded file becomes active, and its heavy
            # data goes directly into active_data (not the library).
            st.session_state["active_fingerprint"] = file_fingerprint
            st.session_state["active_data"] = processed
    ...
    # ------------------------------------------------------------
    # Pick which uploaded file is active.
    # ------------------------------------------------------------
    if len(st.session_state["library"]) > 0:
        ...
        # If the user switched to a DIFFERENT document, reprocess it
        # from its cached raw bytes (cheap to store, since we never
        # kept its chunks/embeddings/index around), and drop the
        # previous document's heavy data from memory.
        if st.session_state.get("active_fingerprint") != selected_fingerprint:

            st.session_state["active_fingerprint"] = selected_fingerprint

            entry = st.session_state["library"][selected_fingerprint]

            with st.spinner(f"Loading {entry['name']}..."):
                st.session_state["active_data"] = process_pdf_bytes(
                    entry["raw_bytes"],
                    entry["name"],
                    st.session_state["user_id"],
                    document_id=entry["document_id"],  # reuse existing DB row, don't re-insert
                )
    ...
    # ------------------------------------------------------------
    # Use the active document's heavy data.
    # ------------------------------------------------------------
    if "active_data" in st.session_state:

        active_doc = st.session_state["active_data"]
...
st.markdown("---")
st.caption("EduCast AI • Built with Streamlit • Groq Llama 3.1 • FAISS • Supabase (PostgreSQL)")